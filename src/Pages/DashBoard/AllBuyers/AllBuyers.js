    ...
    const {
      ...
      queryFn: async ({ queryKey }) => {
        ...
        const res = await fetch(
          `https://auto-reseller-api.vercel.app/users/buyers?page=${page}&limit=${buyersPerPage}&sort=${column}&order=${direction}`,
          {
            headers: {
              'content-type': 'application/json',
              authorization: `bearer ${localStorage.getItem('accessToken')}`,
      ...
      const handleDelete = (id) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(`https://auto-reseller-api.vercel.app/users/buyer/${id}`, {
              method: "DELETE",
              headers: {
                'content-type': 'application/json',
                authorization: `bearer ${localStorage.getItem('accessToken')}`
